...
void *thread_receiving(void *socket){
	...
	while(1){
		...
		if(!strncmp(messageReceived,"file/",5)){
			...
			sem_post(&mutex);
		}else if(!strcmp("Server> The admin closed this channel, disconnecting...\n",messageReceived)){
			sending("/fin\n",sock);
			sem_destroy(&mutex);
			sem_destroy(&mutexFile);
			closing(sock);
			exit(EXIT_SUCCESS);
		}else if(!strcmp("Server> You closed this channel, disconnecting...\n",messageReceived)){
			sem_destroy(&mutex);
			sem_destroy(&mutexFile);
			closing(sock);
			exit(EXIT_SUCCESS);
...
int main(int argc, char *argv[]){
  ...
  connection(clientSocket,&serverAddress,serverPort,ip);
	...
	while(strcmp(messageReceived,"Server>Begin")){
		...
		}while(channelInvalid_v2(messageSent,clientSocket));
		receiving_without_printing(messageReceived,clientSocket);
	}
	...
	do{
		typing(messageSent);
		messageSent[strlen(messageSent)-1]='\0';
  ...
  while(1){
		...
		}else if(isSendingFile && !strncmp("/filename ",messageSent,10)){
